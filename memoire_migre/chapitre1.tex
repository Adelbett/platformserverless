...
\tikzstyle{box}=[draw, rounded corners, minimum width=2.6cm, minimum height=0.9cm, align=center, fill=Gray!20]
\node[box] (client) {Client externe};
\node[box, right=1.6cm of client] (kourier) {Kourier\\(ingress gateway)};
\node[box, right=1.6cm of kourier] (svc) {Service\\namespace tenant};
\node[box, above=0.9cm of kourier, fill=Gray!8] (metallb) {MetalLB};
\draw[->, thick] (client) -- node[above, font=\scriptsize]{requête HTTP sur l'IP externe} (kourier);
\draw[->, thick] (kourier) -- node[above, font=\scriptsize]{routage par nom d'hôte} (svc);
\draw[->, dashed] (metallb) -- node[right, font=\scriptsize]{attribue l'IP externe\\au Service Kourier} (kourier);
\end{tikzpicture}
\caption{Chemin d'une requête externe vers une application tenant — MetalLB attribue une IP au Service Kourier, il ne relaie pas lui-même le trafic}
