...
\multirow{4}{6.6cm}{
\centering
En tant qu'exploitant, je veux disposer d'un cluster Kubernetes opérationnel afin d'héberger les composants de la plateforme.
}
&
...
\multirow{4}{6.6cm}{
\centering
En tant qu'exploitant, je veux assurer la connectivité et l'isolation réseau entre les tenants.
}
&
...
\multirow{3}{6.6cm}{
\centering
En tant qu'exploitant, je veux exposer les services Kubernetes vers le réseau externe.
}
&
...
\multirow{3}{6.6cm}{
\centering
En tant qu'exploitant, je veux disposer d'une passerelle d'entrée pour les futures applications Knative.
}
&
...
\subsubsection{Kubernetes : notions fondamentales et application au projet}

Construire une plateforme capable d'exécuter les applications de plusieurs clients sans qu'ils interfèrent entre eux suppose un socle d'orchestration de conteneurs : c'est le rôle de Kubernetes \cite{KubernetesDoc}, qui automatise le placement, le redémarrage et la mise à l'échelle des conteneurs sur un ensemble de machines.

Notions clés utilisées dans ce chapitre :
\begin{itemize}
    \item Cluster : ensemble des machines, ou nœuds, qui exécutent la plateforme.
    \item Control-plane : décide où et comment exécuter chaque charge.
    \item Workers : nœuds qui exécutent réellement les applications.
    \item Pod : plus petit élément déployable, un ou plusieurs conteneurs partageant la même adresse réseau.
    \item Service : donne à un ensemble de Pods une adresse stable.
    \item Namespace : cloisonne des ressources à l'intérieur d'un même cluster.
\end{itemize}

Le cloisonnement par namespace est le mécanisme sur lequel repose toute la multi-tenance de la plateforme : chaque client obtient son propre namespace, isolé des autres clients et de la plateforme elle-même. La version exacte de Kubernetes et du container runtime utilisés n'est pas documentée de façon fiable dans le dépôt et n'est donc pas mentionnée ici.

\paragraph{Choix d'un cluster on-premise.}\mbox{}\par
La plateforme est hébergée sur un cluster bare-metal propre à NextStep IT, plutôt que sur un service Kubernetes managé (EKS, GKE, AKS), cohérent avec l'offre Infrastructure as a Service de l'intégrateur (cf. Chapitre I) et l'absence de budget cloud récurrent pour ce PFE. Ce choix justifie directement la nécessité de MetalLB (section~\ref{sec:metallb}), qu'un cluster managé fournirait nativement.

\subsubsection{Architecture du cluster de la plateforme}

Le cluster utilisé pour le développement et l'exploitation de la plateforme compte trois nœuds ; la répartition des rôles et les adresses IP (table~\ref{tab:cluster-nodes}) sont confirmées par la configuration réseau du backend, notamment sa liste blanche CORS.
...
\hline
vm01 & Plan de contrôle (control-plane) & 10.9.21.223 \\
\hline
vm02 & Nœud de travail (worker) & 10.9.21.224 \\
\hline
vm03 & Nœud de travail (worker) & 10.9.21.225 \\
\hline
\end{tabular}
\caption{Nœuds du cluster Kubernetes de la plateforme}
\label{tab:cluster-nodes}
\end{table}

Les caractéristiques matérielles et le système d'exploitation de chaque nœud ne sont pas documentés de façon fiable. Un audit technique réalisé après une première mise en exploitation confirme cette topologie ainsi que la chaîne CNI, LoadBalancer et Ingress décrite dans ce chapitre. La figure~\ref{fig:cluster-topology} illustre cette topologie et la séparation logique entre l'espace de noms de la plateforme et ceux des tenants.
...
\tikzstyle{box}=[draw, rounded corners, minimum width=3cm, minimum height=0.9cm, align=center, fill=Gray!20]
\node[box] (client) {Client externe};
\node[box, right=1.9cm of client] (metallb) {MetalLB\\(IP externe)};
\node[box, right=1.9cm of metallb] (kourier) {Kourier\\(ingress gateway)};
\node[box, right=1.9cm of kourier] (svc) {Service\\namespace tenant};
...
\begin{itemize}
    \item État des nœuds du cluster.
\end{itemize}
...
\begin{itemize}
    \item Pods d'infrastructure réseau.
\end{itemize}
...
\begin{itemize}
    \item Politiques réseau des tenants.
\end{itemize}
...
\multirow{2}{6.6cm}{
\centering
En tant qu'exploitant, je veux que Knative Serving soit opérationnel sur le cluster.
}
&
...
\multirow{3}{6.6cm}{
\centering
En tant qu'exploitant, je veux que l'espace de noms d'un tenant soit créé automatiquement au premier déploiement.
}
&
...
\end{tikzpicture}
\caption{Chaîne d'intégration du backend à Knative : le backend ne crée que la ressource Service ; Revision, Deployment et Pod en sont dérivés automatiquement par Knative Serving (blocs en pointillés)}
...
\subsubsection{Premier déploiement d'un tenant}

La figure~\ref{fig:tenant-workflow} synthétise l'enchaînement déclenché par le premier déploiement d'un tenant, depuis la création de son espace de noms — qui hérite de la NetworkPolicy default-deny du Sprint 0 — jusqu'à la disponibilité de l'application. Son implémentation précise côté backend est présentée en Release 1.

\begin{figure}[H]
 \centering
    \includegraphics[width=0.85\textwidth]{image/tenant_workflow.png}
    \caption{Du premier déploiement à l'application disponible}
...
\begin{itemize}
    \item Plan de contrôle de Knative Serving.
\end{itemize}
...
\begin{itemize}
    \item Services Knative créés par le backend dans un namespace tenant.
\end{itemize}
...
\multirow{2}{6.6cm}{
\centering
En tant qu'exploitant, je veux disposer d'un cluster Kafka opéré par Strimzi.
}
&
...
\begin{itemize}
    \item Composants du cluster Kafka.
\end{itemize}
...
\begin{itemize}
    \item Ressource Kafka gérée par l'opérateur.
\end{itemize}

La figure~\ref{fig:capture-kafka-cluster} affiche la ressource personnalisée \texttt{Kafka} réconciliée par Strimzi : un courtier et une instance ZooKeeper désirés, à l'état \texttt{READY: True}, en mode ZooKeeper plutôt que KRaft.
...
\begin{itemize}
    \item Points d'accès réseau du cluster Kafka.
\end{itemize}
...
Deux points d'accès \texttt{LoadBalancer} complètent le service de bootstrap interne, sur le port 9093, pour permettre la connexion d'outils de développement externes ; leur configuration est examinée dans l'audit de sécurité du chapitre de validation.
