...
Le tableau~\ref{tab:NextStep} présente la fiche d'identité de NextStep IT.
...
\hline
Adresse & Rue 8612, Rue de l'énergie solaire, I 2035 \\
\hline
Téléphone & +216 70 258 200 \\
\hline
Site Web & https://www.nextstep-it.com/ \\
\hline
Logo & \includegraphics[width=3cm]{image/nexsteplogo.png} \\
\hline
\end{tabular}
...
L'offre de NextStep IT se répartit en six domaines. Ils couvrent la connectivité, l'équipement, l'hébergement et l'accompagnement des systèmes d'information.

\begin{itemize}
    \item \textbf{Fournisseur Internet :}\\
    - Accès Internet haut débit sécurisé.\\
    - Offres calibrées selon les usages de chaque client.\\
    - Réseaux SD-WAN, pour la performance et la continuité de service.

    \item \textbf{Intégration des solutions informatiques :}\\
    - Réseaux d'entreprise : réseaux locaux, Wifi, interconnexions de sites.\\
    - Cybersécurité : VPN, pare-feux, IPS, antivirus, authentification forte.\\
    - Data Center virtualisé, téléphonie IP et visioconférence.

    \item \textbf{Services Cloud :}\\
    - Hébergement et Housing.\\
    - Infrastructure as a Service (IaaS).\\
    - Sauvegarde, reprise après sinistre et collaboration en ligne.

    \item \textbf{Services Managés :}\\
    - Exploitation des infrastructures, réseaux, télécoms et applications.\\
    - Sécurité proactive, outils collaboratifs, haute disponibilité.\\
    - Supervision centralisée depuis des centres NOC et SOC.

    \item \textbf{Support et Assistance technique :}\\
    - Assistance technique et maintenance.\\
    - Diagnostic et résolution des incidents.\\
    - Infogérance des systèmes informatiques.

    \item \textbf{Conseil et accompagnement :}\\
    - Audit des infrastructures existantes.\\
    - Études fonctionnelles et techniques, assistance à maîtrise d'ouvrage (AMOA).\\
    - Aide aux choix stratégiques et technologiques.
\end{itemize}

Notre projet de fin d'études relève du domaine des \textbf{services Cloud}. Il vise à renforcer l'offre \textit{Infrastructure as a Service} (IaaS) existante. Il consiste à concevoir et réaliser une plateforme \textit{Platform as a Service} (PaaS) serverless, bâtie sur Kubernetes. Cette plateforme permet aux clients de déployer et d'exploiter leurs applications conteneurisées sans avoir à gérer eux-mêmes la complexité de l'orchestration sous-jacente.
...
Pionnier du PaaS, Heroku~\cite{Heroku} propose une expérience de déploiement très simplifiée (git push pour déployer), mais reste une plateforme propriétaire et fermée, offrant peu de contrôle sur l'infrastructure sous-jacente et dont les coûts croissent rapidement à l'échelle.

\subsubsection*{Google Cloud Run}
Représentative des offres serverless managées des fournisseurs de cloud public, au même titre qu'AWS Lambda ou Azure Container Apps. Cloud Run offre une excellente mise à l'échelle automatique, y compris le retour à zéro instance, mais impose un verrouillage fort vis-à-vis du fournisseur (vendor lock-in) et ne peut pas être hébergée sur une infrastructure privée ou on-premise, ce qui pose un problème pour un intégrateur comme NextStep IT souhaitant proposer une offre maîtrisée sur son propre cluster.

\subsubsection*{OpenShift (Red Hat)}
Plateforme PaaS complète bâtie sur Kubernetes, très robuste mais lourde à opérer et sous licence commerciale~\cite{OpenShift}, avec un coût d'entrée élevé peu compatible avec une offre destinée à des PME.

\subsection*{Comparaison des solutions existantes}
Le tableau~\ref{tab:comparaison_paas} confronte ces trois solutions aux critères issus de la problématique.

\renewcommand{\arraystretch}{1.3}
\begin{longtable}{|p{5.2cm}|p{2.8cm}|p{2.8cm}|p{2.8cm}|}
\caption{Comparaison des solutions PaaS et serverless existantes}
\label{tab:comparaison_paas}\\
\hline
\textbf{Critères} & \textbf{Heroku} & \textbf{Cloud Run} & \textbf{OpenShift} \\
\hline
\endfirsthead

\multicolumn{4}{l}{\small\itshape (suite du tableau~\ref{tab:comparaison_paas})}\\
\hline
\textbf{Critères} & \textbf{Heroku} & \textbf{Cloud Run} & \textbf{OpenShift} \\
\hline
\endhead
...
\hline
\multicolumn{4}{r}{\small\itshape (suite page suivante)}\\
\endfoot

\hline
\endlastfoot

Déploiement sans expertise Kubernetes & Oui & Oui & Partiel \\
\hline
Mise à l'échelle automatique & Oui & Oui & Oui \\
\hline
Retour à zéro instance & Partiel & Oui & Oui \\
\hline
Hébergement sur infrastructure privée & Non & Non & Oui \\
\hline
Isolation multi-tenant & Oui & Oui & Oui \\
\hline
Intégration événementielle native & Partiel & Oui & Oui \\
\hline
Facturation à l'usage intégrée & Oui & Oui & Non \\
\hline
Portail client dédié & Oui & Oui & Oui \\
\hline
Console d'administration & Non & Non & Oui \\
\hline
Code source ouvert & Non & Non & Partiel \\
\hline
Indépendance vis-à-vis du fournisseur & Faible & Faible & Moyenne \\
\hline
Modèle économique & Payant & Payant (à l'usage) & Payant (licence) \\
\hline
\end{longtable}
\subsubsection*{Conclusion de l'étude de l'existant}
La lecture du tableau~\ref{tab:comparaison_paas} fait apparaître qu'aucune des trois solutions ne réunit simultanément les deux exigences centrales du projet : l'hébergement sur l'infrastructure propre de l'intégrateur, et une facturation à l'usage intégrée. Heroku et Cloud Run offrent le modèle économique recherché mais imposent un hébergement chez le fournisseur ; OpenShift autorise l'hébergement privé mais ne comporte aucun mécanisme de facturation des locataires, et son coût de licence reste peu compatible avec une offre destinée à des PME.
Cette étude met ainsi en évidence un vide entre, d'une part, les briques open source performantes mais non finies (Knative, Strimzi), qui apportent le scale-to-zero et le modèle événementiel sans fournir ni portail, ni gestion multi-tenant, ni facturation, et d'autre part les plateformes commerciales complètes mais fermées ou coûteuses. C'est précisément cet espace que notre projet vise à occuper : construire, au-dessus de briques open source éprouvées, une plateforme PaaS serverless complète, multi-tenant, avec portail applicatif, facturation à l'usage et console d'administration, hébergeable sur l'infrastructure propre de l'intégrateur.
...
\begin{itemize}
    ...
    \item Un backend applicatif développé en Spring Boot~\cite{SpringBootDoc}, exposant une API REST sécurisée, orchestrant le cluster Kubernetes via le client Java Fabric8, et portant la logique métier (gestion des utilisateurs, des applications, de la facturation, des clés d'API, des journaux).
    \item Deux portails web développés en React~\cite{ReactDoc} : un portail client permettant aux locataires de déployer et de superviser leurs applications, et une console d'administration réservée à l'équipe exploitant la plateforme.
    \item Keycloak~\cite{KeycloakDoc} comme fournisseur d'identité, assurant l'authentification et la gestion des rôles (administrateur de la plateforme, administrateur de compte client, membre d'équipe) via le protocole OpenID Connect / OAuth2.
    \item PostgreSQL~\cite{PostgreSQLDoc} comme système de persistance relationnelle pour les entités métier de la plateforme.
    \item Une chaîne CI/CD interne, basée sur Jenkins~\cite{JenkinsDoc} et Kaniko~\cite{KanikoDoc}, non pas un service proposé aux clients de la plateforme, mais l'outillage utilisé par l'équipe de développement elle-même pour construire, publier et déployer automatiquement les différents composants du projet (backend, portail client, console d'administration) au fil de leur évolution, sans nécessiter de démon Docker~\cite{DockerDoc} privilégié.
    \item Une chaîne de supervision basée sur Prometheus~\cite{PrometheusDoc} et Grafana~\cite{GrafanaDoc}, complétée par des mécanismes internes de détection des redémarrages en boucle (crash loops) et d'alerte budgétaire.
\end{itemize}
...
Le choix d'une méthodologie de développement constitue une étape essentielle dans la réussite d'un projet logiciel. Une méthode adaptée permet d'améliorer l'organisation du travail, de faciliter la collaboration entre les différents acteurs et d'assurer une réalisation efficace des fonctionnalités attendues. Le projet combine plusieurs technologies encore peu matures et en évolution rapide (Knative, Strimzi), et certaines contraintes techniques — comportement réel du cluster, limites des outils de supervision — ne pouvaient être pleinement anticipées en amont : une approche agile a donc été privilégiée afin de permettre des ajustements réguliers du périmètre et des priorités tout au long du développement.

\subsection{Méthodologie adoptée}

Nous avons retenu la méthodologie agile Scrum, qui répond le mieux aux besoins de notre projet. Scrum structure le développement en itérations courtes et fixes appelées sprints, à l'issue desquelles un incrément de produit potentiellement livrable est produit. Ce choix se justifie par plusieurs raisons :
...
\begin{figure}[H]
    \centering
    \includegraphics[width=12cm]{image/scrom.png}
