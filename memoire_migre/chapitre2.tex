...
\hline
\endfirsthead
\multicolumn{3}{l}{\small\itshape Suite du tableau~\ref{tab:sb-sprint3}} \\
\hline
User Story & Tâches & Estimation \\
\hline
\endhead
...
& Synchroniser l'utilisateur avec la plateforme. & 2 \\ \cline{2-3}
...
& Intégrer Keycloak au mécanisme d'authentification. & 3 \\ \cline{2-3}
...
& Développer la modification du rôle d'un utilisateur. & 2 \\ \cline{2-3}
& Développer l'interface d'administration (admin-console). & 2 \\ \cline{2-3}
& Tester la gestion des utilisateurs. & 1 \\ \hline

\multirow{5}{6.6cm}{\centering En tant que CLIENT\_ADMIN, je veux gérer mon équipe.}
& Développer l'ajout d'un membre. & 2 \\ \cline{2-3}
& Développer la liste des membres. & 1 \\ \cline{2-3}
& Développer la modification des permissions. & 2 \\ \cline{2-3}
& Développer la suppression d'un membre. & 1 \\ \cline{2-3}
& Développer l'interface de gestion de l'équipe. & 2 \\ \hline
...
Le diagramme réunit les trois acteurs de ce sprint (MEMBER, CLIENT\_ADMIN, ADMIN) et l'ensemble des dix fonctionnalités du sprint.
...
\hline
\endlastfoot
\texttt{POST\slash api\slash auth\slash register} & Créer un compte. \\ \hline
Connexion (Keycloak, OIDC) & Échange des identifiants contre un jeton, hors API du backend. \\ \hline
\texttt{GET\slash api\slash users\slash me} & Consulter son profil. \\ \hline
\texttt{PATCH\slash api\slash users\slash me} & Modifier son profil. \\ \hline
\texttt{GET\slash api\slash users} & Lister les utilisateurs (ADMIN). \\ \hline
\texttt{PATCH\slash api\slash users\slash\{id\}\slash role} & Modifier le rôle d'un utilisateur (ADMIN). \\ \hline
\texttt{GET\slash api\slash team\slash members} & Lister les membres de son équipe. \\ \hline
\texttt{POST\slash api\slash team\slash members} & Ajouter un membre. \\ \hline
\texttt{PUT\slash api\slash team\slash members\slash\{id\}\slash permissions} & Modifier les permissions d'un membre. \\ \hline
\texttt{DELETE\slash api\slash team\slash members\slash\{id\}} & Supprimer un membre. \\ \hline
\end{longtable}}
...
Fonctionnalité : Créer un compte. Toute personne peut s'inscrire depuis le portail ; cette auto-inscription crée systématiquement un compte CLIENT\_ADMIN, propriétaire d'un nouveau tenant. Un compte MEMBER n'est jamais créé par auto-inscription : il est ajouté ultérieurement par un CLIENT\_ADMIN au sein de son équipe (fonctionnalité « Gérer son équipe », détaillée plus loin dans ce sprint).
...
& Développer l'API de déploiement. & 3 \\* \cline{2-3}
& Développer le formulaire de déploiement du portail. & 1 \\* \cline{2-3}
& Intégrer la création de la ressource Knative correspondante. & 2 \\* \cline{2-3}
...
& Synchroniser le statut en temps réel depuis Kubernetes. & 3 \\ \hline
\pagebreak
...
& Développer la mise à jour et le redéploiement. & 2 \\* \cline{2-3}
...
& Développer la liste des révisions. & 2 \\* \cline{2-3}
& Développer le retour à une révision antérieure. & 2 \\* \cline{2-3}
...
& Développer la suspension et la restauration d'une application. & 2 \\* \cline{2-3}
...
\hline
\endlastfoot
\texttt{POST\slash api\slash apps} & Déployer une application. \\ \hline
\texttt{GET\slash api\slash apps} & Lister ses applications. \\ \hline
\texttt{GET\slash api\slash apps\slash\{id\}} & Détail d'une application. \\ \hline
\texttt{POST\slash api\slash apps\slash\{id\}\slash deploy} & Redéployer une application existante. \\ \hline
\texttt{PUT\slash api\slash apps\slash\{id\}} & Modifier une application. \\ \hline
\texttt{DELETE\slash api\slash apps\slash\{id\}} & Supprimer une application. \\ \hline
\texttt{GET\slash api\slash apps\slash\{id\}\slash revisions} & Lister les révisions. \\ \hline
\texttt{POST\slash api\slash apps\slash\{id\}\slash rollback\slash\{revisionName\}} & Revenir à une révision antérieure. \\ \hline
\texttt{GET\slash api\slash admin\slash apps} & Lister toutes les applications, tous locataires confondus (ADMIN). \\ \hline
\texttt{DELETE\slash api\slash admin\slash apps\slash\{id\}} & Suppression forcée (ADMIN). \\ \hline
\texttt{POST\slash api\slash admin\slash apps\slash\{id\}\slash suspend} & Suspendre une application (ADMIN). \\ \hline
\texttt{POST\slash api\slash admin\slash apps\slash\{id\}\slash restore} & Restaurer une application suspendue (ADMIN). \\ \hline
\end{longtable}}
...
\begin{figure}[H]
 \centering
    \includegraphics[width=\textwidth,height=0.92\textheight,keepaspectratio]{image/seq-deploy.png}
...
\begin{figure}[H]
 \centering
    \includegraphics[width=\textwidth]{image/Liste des applications et de leur statut.png}
...
\begin{figure}[H]
 \centering
    \includegraphics[width=0.9\textwidth]{image/capture_adminapps.png}
    \caption{Actions administratives sur une application}
    \label{fig:capture-adminapps}
\end{figure}
...
\multicolumn{3}{l}{\small\itshape (suite du tableau~\ref{tab:sb-sprint5})}\\
\hline
User Story & Tâches & Estimation \\
\hline
\endhead
...
& Développer la création d'une KafkaSource. & 3 \\* \cline{2-3}
& Développer la création d'un Trigger. & 2 \\* \cline{2-3}
& Développer la suppression d'un Trigger. & 1 \\* \cline{2-3}
& Développer la page de configuration des déclencheurs. & 2 \\* \cline{2-3}
& Tester le déclenchement d'une application. & 2 \\ \hline
...
& Développer l'API de publication d'un événement. & 2 \\* \cline{2-3}
...
& Développer la vue globale des sujets, sources et déclencheurs. & 2 \\* \cline{2-3}
...
\hline
\endlastfoot
\texttt{POST\slash api\slash kafka\slash topics} & Créer un sujet Kafka. \\ \hline
\texttt{GET\slash api\slash kafka\slash topics} & Lister ses sujets. \\ \hline
\texttt{GET\slash api\slash kafka\slash topics\slash\{id\}} & Détail d'un sujet. \\ \hline
\texttt{DELETE\slash api\slash kafka\slash topics\slash\{id\}} & Supprimer un sujet. \\ \hline
\texttt{POST\slash api\slash eventing\slash sources} & Créer une KafkaSource. \\ \hline
\texttt{GET\slash api\slash eventing\slash sources} & Lister ses KafkaSource. \\ \hline
\texttt{POST\slash api\slash eventing\slash triggers} & Créer un déclencheur (Trigger). \\ \hline
\texttt{GET\slash api\slash eventing\slash triggers} & Lister ses déclencheurs. \\ \hline
\texttt{DELETE\slash api\slash eventing\slash triggers\slash\{id\}} & Supprimer un déclencheur. \\ \hline
\texttt{POST\slash api\slash events} & Publier un événement (CloudEvent). \\ \hline
\texttt{GET\slash api\slash admin\slash kafka\slash topics} & Lister tous les sujets, tous locataires confondus (ADMIN). \\ \hline
\texttt{DELETE\slash api\slash admin\slash kafka\slash topics\slash\{id\}} & Suppression forcée d'un sujet (ADMIN). \\ \hline
\texttt{GET\slash api\slash admin\slash eventing\slash sources\slash triggers} & Vue globale des sources et déclencheurs (ADMIN). \\ \hline
\end{longtable}}
...
\hline
Scénario nominal & 1. L'utilisateur sélectionne un sujet Kafka et une application cible, avec un filtre sur le type d'événement optionnel. 2. Le backend crée la ressource KafkaSource rattachée au sujet. 3. Le backend crée la ressource Trigger associée, pointant vers l'URL du service Knative de l'application ; le filtre sur le type d'événement n'est ajouté à la ressource que si l'utilisateur en a précisé un, l'absence de filtre laissant passer tous les événements du sujet. 4. La configuration est persistée et son état est renvoyé à l'utilisateur. \\
\hline
...
\end{tikzpicture}
\caption{Processus événementiel Kafka vers Knative}
...
\begin{figure}[H]
 \centering
    \includegraphics[width=\textwidth,height=0.92\textheight,keepaspectratio]{image/seq-eventing-trigger.png}
...
\multicolumn{3}{l}{\small\itshape (suite du tableau~\ref{tab:sb-sprint6})}\\
\hline
User Story & Tâches & Estimation \\
\hline
\endhead
...
& Développer le flux temps réel des journaux de déploiement. & 3 \\*
\cline{2-3}
& Développer le flux temps réel des journaux bruts du conteneur. & 2 \\ \hline
...
& Développer les métriques par application. & 2 \\* \cline{2-3}
& Développer le flux temps réel des métriques. & 2 \\* \cline{2-3}
...
\hline
\endlastfoot
\texttt{GET\slash api\slash logs\slash apps\slash\{id\}} & Journaux de déploiement d'une application, filtrables par niveau. \\ \hline
\texttt{GET\slash api\slash logs\slash me} & Ses propres journaux. \\ \hline
\texttt{GET\slash api\slash logs\slash stream} (SSE) & Flux temps réel des journaux de déploiement. \\ \hline
\texttt{GET\slash api\slash logs\slash apps\slash\{id\}\slash pod-logs\slash stream} (SSE) & Flux temps réel des journaux bruts du conteneur. \\ \hline
\texttt{GET\slash api\slash metrics\slash apps\slash\{id\}} & Métriques d'une application (requêtes/s, latence, CPU, mémoire). \\ \hline
\texttt{GET\slash api\slash metrics\slash apps\slash\{id\}\slash stream} (SSE) & Flux temps réel des métriques d'une application. \\ \hline
\texttt{GET\slash api\slash admin\slash logs} & Journaux de déploiement de tous les locataires (ADMIN). \\ \hline
\end{longtable}}
...
\begin{figure}[H]
 \centering
    \includegraphics[width=\textwidth,height=0.92\textheight,keepaspectratio]{image/seq-logs.png}
