...
Le diagramme réunit CLIENT\_ADMIN et MEMBER (sous permission \texttt{MANAGE\_KAFKA}/\texttt{MANAGE\_EVENTING}), ADMIN, et un acteur Système externe pour la publication d'événements via clé d'API, avec l'ensemble des fonctionnalités du sprint.

\EmplacementFigure{13cm}{8cm}{Diagramme de cas d'utilisation du Sprint 5 — Gestion de la messagerie événementielle}{fig:uc-sprint5}
...
\texttt{POST /api/events} & Publier un événement (CloudEvent), par jeton ou par clé d'API. \\ \hline
...
Deux fonctionnalités sont détaillées : la création d'un sujet Kafka, brique de base du domaine ; la configuration d'un déclencheur, la plus riche techniquement puisqu'elle relie deux briques indépendantes du socle (Kafka et Knative).

\subsubsection{Cas d'utilisation : « Créer un sujet Kafka »}
...
\begin{longtable}{|p{4cm}|p{11cm}|}
\hline
Nom & Créer un sujet Kafka \\
\hline
Acteur & CLIENT\_ADMIN, MEMBER (avec permission de gestion Kafka) \\
\hline
Pré-condition & L'utilisateur est authentifié et dispose de la permission requise \\
\hline
Scénario nominal & 1. L'utilisateur soumet un nom de sujet depuis la page de gestion Kafka. 2. Le backend crée la ressource correspondante, rattachée au cluster Kafka du tenant. 3. Le sujet créé est renvoyé à l'utilisateur, affiché dans la liste, et devient disponible pour la configuration d'un déclencheur. \\
\hline
Post-condition & Le sujet est disponible pour publication et pour association à une application. \\
\hline
...
\EmplacementFigure{13cm}{6cm}{Diagramme de séquence système — Créer un sujet Kafka}{fig:seqsys-createtopic}

\subsubsection*{Diagramme de séquence objet}

\EmplacementFigure{13cm}{7cm}{Diagramme de séquence objet — Créer un sujet Kafka}{fig:seqobj-createtopic}
...
\EmplacementFigure{13cm}{6cm}{Diagramme de séquence système — Configurer un déclencheur d'événement}{fig:seqsys-eventing}

\subsubsection*{Diagramme de séquence objet}

\EmplacementFigure{15cm}{9cm}{Diagramme de séquence objet — Configurer un déclencheur d'événement}{fig:seqobj-eventing}
...
\EmplacementFigure{13cm}{8cm}{Interface de gestion des sujets Kafka}{fig:capture-kafkatopics}

Fonctionnalité : Configurer un déclencheur d'événement. La page de configuration des déclencheurs permet de sélectionner un sujet Kafka et une application, puis de créer le déclencheur associé.

\EmplacementFigure{13cm}{8cm}{Interface de configuration d'un déclencheur}{fig:capture-eventing-ui}
...
Fonctionnalité : Publier un événement. Un utilisateur, ou un système externe muni d'une clé d'API, publie un événement directement sur un sujet.

\EmplacementFigure{13cm}{8cm}{Publication d'un événement}{fig:capture-publish}
...
\EmplacementFigure{13cm}{8cm}{Vue administrateur des sujets, sources et déclencheurs}{fig:capture-admineventing}
...
\EmplacementFigure{13cm}{8cm}{Diagramme de cas d'utilisation du Sprint 6 — Gestion de l'observabilité applicative}{fig:uc-sprint6}
...
\EmplacementFigure{13cm}{6cm}{Diagramme de séquence système — Consulter les journaux en temps réel}{fig:seqsys-logs}

\subsubsection*{Diagramme de séquence objet}

\EmplacementFigure{15cm}{8cm}{Diagramme de séquence objet — Consulter les journaux en temps réel}{fig:seqobj-logs}

\subsubsection{Cas d'utilisation : « Consulter les métriques d'une application »}
...
\begin{longtable}{|p{4cm}|p{11cm}|}
\hline
Nom & Consulter les métriques d'une application \\
\hline
Acteur & CLIENT\_ADMIN, MEMBER (avec permission de consultation du monitoring) \\
\hline
Pré-condition & L'application appartient au compte de l'utilisateur (ou de son CLIENT\_ADMIN) \\
\hline
Scénario nominal & 1. L'utilisateur ouvre la page de monitoring de son application. 2. Le backend interroge Prometheus pour les métriques de la révision Knative concernée. 3. Les métriques (requêtes par seconde, latence, CPU, mémoire) sont affichées et actualisées en continu. \\
\hline
Post-condition & L'utilisateur dispose d'une vue à jour de la charge de son application. \\
\hline
\caption{Description textuelle du cas d'utilisation « Consulter les métriques d'une application »}
\label{tab:uc-metrics}
\end{longtable}

\subsubsection*{Diagramme de séquence système}

\EmplacementFigure{13cm}{6cm}{Diagramme de séquence système — Consulter les métriques d'une application}{fig:seqsys-metrics}

\subsubsection*{Diagramme de séquence objet}

\EmplacementFigure{13cm}{7cm}{Diagramme de séquence objet — Consulter les métriques d'une application}{fig:seqobj-metrics}
...
\EmplacementFigure{13cm}{8cm}{Interface de consultation des journaux de déploiement}{fig:capture-deploylogs}
...
\EmplacementFigure{13cm}{8cm}{Flux temps réel des journaux applicatifs}{fig:capture-livelogs}

Fonctionnalité : Consulter les métriques d'une application. La page de monitoring affiche les métriques CPU, mémoire et requêtes par seconde de l'application, issues de Prometheus.

\EmplacementFigure{13cm}{8cm}{Interface de monitoring d'une application}{fig:capture-monitoring}
...
\EmplacementFigure{13cm}{8cm}{Vue administrateur des journaux}{fig:capture-adminlogs}

\paragraph{Précision sur les journaux et Elasticsearch.}\mbox{}\par
La bibliothèque \texttt{spring-boot-starter-data-elasticsearch} figure parmi les dépendances du backend, mais aucune classe du code source ne l'utilise réellement (pas de dépôt, d'entité indexée ni d'appel à un client Elasticsearch) : les journaux applicatifs ne transitent donc pas par Elasticsearch dans ce projet, contrairement à ce que la présence de cette dépendance pourrait laisser supposer. Le chemin réel est direct : API Kubernetes $\rightarrow$ service de journaux $\rightarrow$ diffusion en temps réel $\rightarrow$ portail client.
