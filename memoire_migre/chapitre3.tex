...
\multirow{3}{6.6cm}{\centering En tant que CLIENT\_ADMIN, je veux consulter mon historique d'usage et de facturation afin de suivre mes dépenses.}
...
\multirow{4}{6.6cm}{\centering En tant que CLIENT\_ADMIN, je veux enregistrer un moyen de paiement et régler mes factures afin de rester en règle.}
...
\multirow{1}{6.6cm}{\centering En tant que CLIENT\_ADMIN, je veux exporter un rapport de facturation afin de le transmettre à ma comptabilité.}
& Développer l'export du rapport de facturation. & 1 \\ \hline

\multirow{3}{6.6cm}{\centering En tant qu'ADMIN, je veux consulter la facturation de tous les clients et générer les factures mensuelles afin de piloter la facturation globale.}
...
\multirow{3}{6.6cm}{\centering En tant qu'ADMIN, je veux relancer et suspendre les clients en situation d'impayé afin de limiter les pertes.}
& Développer les relances avant échéance. & 1 \\ \cline{2-3}
& Développer la suspension pour non-paiement. & 1 \\ \cline{2-3}
...
\EmplacementFigure{13cm}{8cm}{Diagramme de cas d'utilisation du Sprint 7 — Gestion de la facturation et des paiements}{fig:uc-sprint7}
...
Deux fonctionnalités sont détaillées : la consultation de sa facturation, la plus fréquemment utilisée ; le paiement d'une facture, techniquement la plus riche puisqu'elle implique un prestataire externe.

\subsubsection{Cas d'utilisation : « Consulter sa facturation »}
...
\hline
Nom & Consulter sa facturation \\
\hline
Acteur & CLIENT\_ADMIN, MEMBER (avec permission de consultation de la facturation) \\
\hline
Pré-condition & Le compte dispose d'au moins une application ayant fait l'objet d'un instantané de consommation \\
\hline
Scénario nominal & 1. L'utilisateur ouvre la page de facturation du portail. 2. Le backend agrège les instantanés horaires et les factures du compte. 3. Le portail affiche le coût projeté du mois en cours ainsi que l'historique des factures. \\
\hline
Post-condition & L'utilisateur dispose d'une vue à jour de sa consommation et de ses factures. \\
\hline
\caption{Description textuelle du cas d'utilisation « Consulter sa facturation »}
\label{tab:uc-billing}
\end{longtable}

\subsubsection*{Diagramme de séquence système}

\EmplacementFigure{13cm}{6cm}{Diagramme de séquence système — Consulter sa facturation}{fig:seqsys-billing}

\subsubsection*{Diagramme de séquence objet}

\EmplacementFigure{13cm}{7cm}{Diagramme de séquence objet — Consulter sa facturation}{fig:seqobj-billing}
...
\EmplacementFigure{13cm}{6cm}{Diagramme de séquence système — Payer une facture}{fig:seqsys-pay}

\subsubsection*{Diagramme de séquence objet}

\EmplacementFigure{14cm}{8cm}{Diagramme de séquence objet — Payer une facture}{fig:seqobj-pay}

\subsection{Réalisation du Sprint 7}

Fonctionnalité : Consulter sa facturation. La page de facturation du portail affiche le coût projeté du mois en cours et l'historique des factures.

\EmplacementFigure{13cm}{8cm}{Interface de facturation du compte client}{fig:capture-billing}

Fonctionnalité : Enregistrer un moyen de paiement et payer une facture. Le CLIENT\_ADMIN enregistre un moyen de paiement et règle ses factures depuis la même page.

\EmplacementFigure{13cm}{8cm}{Interface de paiement}{fig:capture-payment}

Fonctionnalité : Exporter un rapport de facturation. L'utilisateur exporte son historique de facturation.

\EmplacementFigure{13cm}{8cm}{Export du rapport de facturation}{fig:capture-billingexport}
...
\EmplacementFigure{13cm}{8cm}{Interface de facturation globale (console d'administration)}{fig:capture-adminbilling}

Fonctionnalité : Relancer et suspendre les clients en situation d'impayé (ADMIN). L'ADMIN consulte les factures impayées et suspend, le cas échéant, l'application concernée.

\EmplacementFigure{13cm}{8cm}{Interface de suivi des impayés}{fig:capture-overdue}
...
\EmplacementFigure{12cm}{7cm}{Diagramme de cas d'utilisation du Sprint 8 — Comptes clients, quotas et audit}{fig:uc-sprint8}
...
\EmplacementFigure{13cm}{6cm}{Diagramme de séquence système — Suspendre un compte client}{fig:seqsys-suspend}

\subsubsection*{Diagramme de séquence objet}

\EmplacementFigure{13cm}{7cm}{Diagramme de séquence objet — Suspendre un compte client}{fig:seqobj-suspend}

\subsubsection{Cas d'utilisation : « Consulter et exporter le journal d'audit »}
...
\begin{longtable}{|p{4cm}|p{11cm}|}
\hline
Nom & Consulter et exporter le journal d'audit \\
\hline
Acteur & ADMIN \\
\hline
Pré-condition & Des actions administratives ont déjà été journalisées \\
\hline
Scénario nominal & 1. L'ADMIN ouvre le journal d'audit. 2. Il filtre les actions par type ou par période. 3. Il exporte, le cas échéant, la vue filtrée au format CSV. \\
\hline
Post-condition & L'ADMIN dispose d'une trace consultable ou exportable des actions administratives. \\
\hline
\caption{Description textuelle du cas d'utilisation « Consulter et exporter le journal d'audit »}
\label{tab:uc-audit}
\end{longtable}

\subsubsection*{Diagramme de séquence système}

\EmplacementFigure{13cm}{6cm}{Diagramme de séquence système — Consulter le journal d'audit}{fig:seqsys-audit}

\subsubsection*{Diagramme de séquence objet}

\EmplacementFigure{13cm}{7cm}{Diagramme de séquence objet — Consulter le journal d'audit}{fig:seqobj-audit}

\subsection{Réalisation du Sprint 8}

Fonctionnalité : Lister et suspendre ou réactiver un compte client. La console d'administration liste les comptes clients avec leur état, et permet de les suspendre ou de les réactiver.

\EmplacementFigure{13cm}{8cm}{Interface de gestion des comptes clients}{fig:capture-adminclients}

Fonctionnalité : Consulter et modifier le quota d'un client. Le quota de ressources d'un client (CPU, mémoire, nombre maximal d'applications) est consultable et modifiable, et propagé au cluster.

\EmplacementFigure{13cm}{8cm}{Interface de gestion des quotas}{fig:capture-quota}
...
\EmplacementFigure{13cm}{8cm}{Interface du journal d'audit}{fig:capture-auditlog}
...
& Développer la consultation des nœuds, pods, namespaces et volumes. & 1 \\ \cline{2-3}
& Développer la consultation des services Knative et des courtiers Kafka. & 1 \\ \cline{2-3}
& Développer la vue d'ensemble consolidée du cluster. & 1 \\ \hline
...
\multirow{3}{6.6cm}{\centering En tant que visiteur, je veux consulter le statut public de la plateforme afin de connaître sa disponibilité.}
& Développer la consultation du statut par composant. & 1 \\ \cline{2-3}
& Développer l'historique des incidents. & 1 \\ \cline{2-3}
& Limiter le débit de ces points d'accès publics. & 1 \\ \hline
...
\EmplacementFigure{13cm}{8cm}{Diagramme de cas d'utilisation du Sprint 9 — Exploitation du cluster et statut public}{fig:uc-sprint9}
...
\texttt{GET /api/status} & Statut public par composant, débit limité. \\ \hline
\texttt{GET /api/status/incidents} & Historique des incidents, débit limité. \\ \hline
\end{longtable}
...
\EmplacementFigure{13cm}{6cm}{Diagramme de séquence système — Superviser le cluster}{fig:seqsys-cluster}

\subsubsection*{Diagramme de séquence objet}

\EmplacementFigure{13cm}{7cm}{Diagramme de séquence objet — Superviser le cluster}{fig:seqobj-cluster}

\subsubsection{Cas d'utilisation : « Consulter le statut public »}
...
\begin{longtable}{|p{4cm}|p{11cm}|}
\hline
Nom & Consulter le statut public de la plateforme \\
\hline
Acteur & Public non authentifié (également accessible aux trois autres rôles) \\
\hline
Pré-condition & Aucune \\
\hline
Scénario nominal & 1. Le visiteur ouvre la page de statut public. 2. Le backend interroge l'état des composants de la plateforme. 3. La disponibilité par composant et l'historique des incidents sont affichés. \\
\hline
Post-condition & Le visiteur connaît l'état de disponibilité de la plateforme, sans avoir besoin de s'authentifier. \\
\hline
\caption{Description textuelle du cas d'utilisation « Consulter le statut public »}
\label{tab:uc-status}
\end{longtable}

\subsubsection*{Diagramme de séquence système}

\EmplacementFigure{13cm}{6cm}{Diagramme de séquence système — Consulter le statut public}{fig:seqsys-status}

\subsubsection*{Diagramme de séquence objet}

\EmplacementFigure{13cm}{7cm}{Diagramme de séquence objet — Consulter le statut public}{fig:seqobj-status}
...
\EmplacementFigure{13cm}{8cm}{Interface de supervision du cluster}{fig:capture-cluster}
...
\EmplacementFigure{13cm}{8cm}{Interface des alertes et événements du cluster}{fig:capture-alerts}

Fonctionnalité : Consulter le statut public. Un visiteur non authentifié consulte la disponibilité de la plateforme par composant et l'historique des incidents.

\EmplacementFigure{13cm}{8cm}{Page de statut public de la plateforme}{fig:capture-status}
