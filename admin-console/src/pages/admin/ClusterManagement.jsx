...
import {
    ...
    Database, GitBranch, CheckCircle, XCircle, AlertCircle, Trash2, Activity,
} from 'lucide-react';
...
const ClusterManagement = () => {
    ...
    // Apps are now managed on their own page (Sidebar "Apps" -> /apps,
    // AdminApps.jsx) — this view keeps the raw app count for the header
    // summary and StatCards only.
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24, paddingBottom: 40 }}>
            ...
            <div>
                <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid #1F2B3A', marginBottom: 20 }}>
                    {TABS.map(tab => {
                        const count = { pods: pods.length, knative: knSvcs.length, kafka: topics.length, eventing: sources.length + triggers.length }[tab.id];
                        return (
